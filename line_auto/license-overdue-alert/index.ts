...
Deno.serve(async (req) => {
  ...
  if (totalUnsub > 0) {
    let text = `🪪 *免許証 未提出アラート（全店・当日〜2日先）* — 合計 *${totalUnsub}件*\n免許証をまだアップしていないお客様です。各店の免許証チェックリストで対応（連絡/消し込み）をお願いします。\n`;
    for (const r of results) {
      if (!r.unsub || !r.unsub.length) continue;
      text += `\n*${r.label}｜未提出 ${r.unsub.length}件*\n`;
      text += r.unsub.slice(0, 20).map((u: any) => `・${u.date} ${u.time}　${u.name}`).join("\n");
      if (r.unsub.length > 20) text += `\n…ほか ${r.unsub.length - 20}件`;
      text += "\n";
    }
    await slack(text);
  }
  return json({ ok: true, totalUnsub, results });
});
