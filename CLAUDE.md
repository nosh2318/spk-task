...
## 💳 2026-09-25 高松じゃらん事前決済＝HANDYMAN高松のみ(.com)・BUDDICAでは絶対送らない／新経路の初動メール・決済リンク未発行を根治(オーナー厳命・恒久)
**大原則(オーナー厳命)：じゃらん高松＝HANDYMAN高松空港店のみ(rent-handyman.com/.com)。BUDDICA TOURISMブランドでは絶対に送らない。BUDDICAにじゃらんは無い。** 高松HDMじゃらんの正しい決済フロー＝**予約完了→初動welcomeメール(HANDYMAN .com)にSquare事前決済リンクを添付→出発前まで追跡**（那覇/札幌と同じ事前決済）。送信元＝`reserve@rent-handyman.com`（`hdm-tkm-send-mail` FROM・確認済）。
- **バグ根治(直近予約でリンク未発行)**：直近じゃらん高松(R0CMD8L3/R0TLG70O/R0OZ2JKB)が`mail=""`で取込まれ、初動メールcron`hdm-tkm-enqueue`が**mail必須**で除外→welcome未送信→リンク未発行だった。原因＝新経路(reserve.com→札幌GAS→**bt-import EF**)が`mail`/`tel`/決済リンクを書いていなかった(じゃらんメールにメアドはあるのにEFのrowにmail欄が無く落ちていた)。→ **bt-import EF(`~/spk-task/line_auto/bt-import/index.ts`→deploy`~/hdm-car-delivery/supabase/functions/bt-import/`)に`mail:r.mail||"",tel:r.tel||""`追加＋じゃらんは`payment:"じゃらん事前決済(Square)",paid:false`(楽天/skyticket=事前カード決済済は据置)**。これで今後の高松じゃらんは取込時にメアドが入り、hdm-tkm-enqueueが自動でwelcome+リンクを送る。
- **BUDDICA側でじゃらんを送らせない**：`gas_torikomi_clasp/コード.js`(buddicatourism読取)の`checkBtPaymentReminders`/`checkBtUnpaidAlert`は**brand無条件**で`bt_reservations paid=false&pay_url有`を拾い「【BUDDICA TOURISM 高松空港店】お支払い再送」を送る作りだった＝pay_url設定でじゃらんに誤発火。→ 両クエリに`&brand=eq.BUDDICA`追加＋`sendBtInitialPayment_`冒頭に`if(ota==='じゃらん'||brand==='HDM')return`ガード(clasp push済)。**じゃらん/HDMには二度とBUDDICAブランドが飛ばない**。
- **Square決済リンク作成(手動/緊急時の型)**：`POST connect.squareup.com/v2/online-checkout/payment-links`・`quick_pay`・**location `L8N7J9RKPN3WH`(HDM Square＝北海道銀行*670・BT/KEYDROP/官方と同一口座)**・token＝`~/outputs/handyman-receipt-bot/Code.gs`の`SQUARE_API_TOKEN`(EAAA…・同口座)・`price_money{amount:利用者への請求額(JPY整数),currency:JPY}`・name＝`高松空港店 <氏名>様（<予約番号>） じゃらん決済 M/D-M/D`。**金額は必ず"利用者への請求額"(ポイント/クーポン後)を実メールで検証してから**(誤請求防止)。作成後`bt_reservations`に`pay_url`/`square_order_id`、`bt_jalan_payments`に`status/square_payment_url/square_order_id/link_created_at/email_sent_at/amount/customer_email`。**入金消し込み＝payment-webhookが`square_order_id`で突合→paid化**。既存GAS関数＝`gas_torikomi createBtSquarePaymentLink_`/`sendBtInitialPayment_`(ただしBUDDICAブランド送信＝じゃらんには使わない)。
- **HANDYMAN .com からテスト/任意送信する型(一時EF)**：`RESEND_API_KEY`はBT secretでローカル不可→**一時EF `hdm-tkm-test-send`(BT project・`--no-verify-jwt`・secrets RESEND_API_KEY/HDM_TKM_FROM/CRON_SECRETを継承)** を作り`{secret,to,subject,text}`を`reserve@rent-handyman.com`から送信→検証→**使用後に必ず削除**(`functions delete`)。テンプレ正本＝`hdm_tkm_templates`(key=welcome/_payblock/_footer)。マイページ＝`https://nosh2318.github.io/spk-task/my-tkm.html?t=<mypage_token>`。二重送信防止＝`hdm_tkm_notifications`の`log_key='welcome:'+予約番号`(enqueueは`resolution=ignore-duplicates`)＝直接送信したら同log_keyでwelcome sent行を入れておく。
- **「未送信なのに入金済み」の矛盾＝表示欠陥(実害なし)**：入金済み予約は初動welcomeでリンクが届いて決済済(`hdm_tkm_notifications` welcome=sent)。`bt_jalan_payments.email_sent_at`は"別系統の催促メール"用で初動メール送信を記録しない＝送信記録が2箇所に分かれていた。今後は初動送信時に`email_sent_at`も入れる運用に(このセッションの3件はそうした)。
- **BT OP決済バッジ根治(v1.0.473-BT)**：高松OPの💳決済バッジ判定3箇所が`t.ota==="J"`固定で、**BT じゃらんは`ota="じゃらん"`(フルネーム)**のため入金済バッジが出なかった(R0TWIKDO=入金済なのに無表示)。→ `t.ota==="J"`→`(t.ota==="J"||t.ota==="じゃらん")`に一括修正。`PayBadge`(サマリ)は元から"じゃらん"対応・`nhaJpMap`は`bt_jalan_payments`をreservation_idで読む。
- **教訓**：①高松は同一`bt_reservations`にHDM(じゃらん/楽天/skyticket/公式=rent-handyman.com)とBUDDICA(たびらい/エアトリ/RDC/自社HP=buddica-tourism.jp)が混在＝**ブランドで送信元・テンプレ・決済系を完全分離**(brand列/ota名で判定)。じゃらん=HDM。②取込EF(bt-import)は必ず`mail`/`tel`を書く(初動メール/決済リンクの前提)。③OTA判定は`ota==="J"`のような札幌形式でなくBTは`"じゃらん"`フルネーム＝バッジ/決済判定は両方拾う。④決済リンクは実メールの「利用者への請求額」で金額検証→テスト送信(オーナー確認)→本送信、の順(誤請求・誤ブランド防止)。⑤bt-importは別セッションと同時編集が起きる＝deploy前に共有台帳確認。
...
## 📥 2026-09-24 reserve受信箱を可視化＝mail-status.html＋問い合わせAPPに「予約受信」を追加（.com高松/.jp札幌那覇 の受信→取込を一目・盲点解消）
オーナー「今どのメールが来て取り込まれたか見えないのが面倒の根本」。**reserve@rent-handyman.com は転送専用で実箱が無いが、転送先 noritaka.oshita@gmail.com の受信箱に実体が届く（To:が保持され `to:reserve@rent-handyman.com` で検索可・実際に高松楽天RC12461296512536367=高松空港店J_TAK が届いている確認済）**＝Gmail読取する札幌GASが記録すれば可視化できる（私の初回「HTMLからは見れない=できない」は不正確・訂正）。
- **仕組み（単一ソース・3点）**：①**台帳`reserve_inbox_mirror`(MAIN・message_id PK・anon SELECT可)** ②**GAS `mirrorReserveInbox_()`(gas/Code.js・processNewEmails末尾でtry/catch呼出＝取込ロジックに触れず記録のみ・例外安全)**＝OTA送信元(otaSenderList_)のnewer_than:3dを走査→To:が`reserve@rent-handyman.com`(高松)/`.jp`(札幌那覇)のみ抽出→ota/予約番号(extractField_)/店(本文`_TAK|高松空港店`or.com→高松)を`on_conflict=message_id`でupsert。clasp push済＝30分毎に自動記録 ③**ビューア2箇所が同じ源を読む＝二重管理なし**：`mail-status.html`の📥受信セクション／`handyman-inquiry.vercel.app`の**「📥予約受信」タブ**(ResvInboxView)。両方 reserve_inbox_mirror＋取込実照合RPCを読む。
...
## 🚨 2026-09-24 【自戒・重大】可視化のためのGmailミラーを取込GASに足して札幌/那覇のGmailクォータを枯渇させ取込を停止させた
オーナー「また意味不明な実装しクォータ消費だろ・台帳みろ」＝完全に正しい。**今朝私が`gas/Code.js`(札幌/那覇取込GAS)の`processNewEmails`(15分毎)末尾に`mirrorReserveInbox_()`を追加**し、その中で`GmailApp.search(fromClause, newer_than:3d, 0, 150)`＝**毎回最大150スレッドを追加検索**（既存の取込検索200スレッドに上乗せ）→ Gmail使用量が倍増し**12:22 JSTにクォータ枯渇→processNewEmails全体が停止→札幌/那覇の新規取込・キャンセル処理が全部止まった**。裏取り＝`reserve_inbox_mirror`が06:06(投入直後)〜12:23で記録が途切れ、まさに取込停止時刻と一致。
- **根治**：`mirrorReserveInbox_()`の呼び出しを撤去（可視化目的のミラーは取込本体のクォータを食うべきでない）。commit・要オーナー貼付。
- **最大の失敗＝台帳を見ず「私は原因でない」と断言した**：午後のリレー作業(別アカウント)だけ見て、朝の自分の追加(同アカウント・同GAS)を見落とし「12:56以降・別アカウントだから私でない」と誤って断定→オーナーに「台帳みろ」と正された。**アクション台帳(HANDYMAN_AGENT_LEDGER.jsonl)を先に見ればgas/Code.js編集(05:57-07:03)が即分かった**。記憶/推論で話さず必ず台帳を先に見る(最上位ルールの実地違反)。
- **恒久教訓(クラス)**：①**取込/決済など"本番を動かすGAS"に、可視化・ログ・監視目的のGmail多用処理(GmailApp.search)を足さない**。Gmailクォータは同一Googleアカウントで全トリガー共有＝1つの重い検索が本体を殺す。可視化が要るなら別アカウント/別手段(pg_cron/DB)でやる。②「取込には触れず記録のみ・例外安全」と書いても、Gmail検索を足した時点で本体のクォータを奪う＝"独立"ではない。③自分の変更が原因かは必ず台帳(いつ・どのファイル)で確認してから発言する。
...
## ⚠️ 2026-09-24 高松リレー「HP直販の自動取込が8回失敗」アラート＝誤報の根治（既存/非高松skipを失敗扱いしていた）
#reservation_notification-高松空港店に「⚠️HP直販の自動取込が8回失敗」が出た件（件名=ご予約完了のお知らせ×2）。**実害ゼロ＝誤報**（site予約は全件bt_reservationsに取込済・漏れなし）。真因＝`gas_relay_clasp/コード.js` の `importReservation_` が**「既存のため取込skip(=既に取込済み・成功扱いすべき)」でも `''` を返す**（line356）→呼び出し側が `!tok` で失敗扱い→8回リトライ→誤アラート。**私が手動投入したNJH37402(小野坂)等が「既存」で返り、誤って8回失敗と通知されていた**。
- **根治**：意図的スキップ（`高松以外skip`/`既存のため取込skip`）は sentinel `'__SKIP__'` を返し、呼び出し側で `if(tok==='__SKIP__'){clearTry_(id)}`＝成功扱い(カウンタ解除・リトライしない)に。真の失敗（DB取込NG line371 / 番号欠落 line353）だけ `''`＝リトライ対象。8回到達アラートに**差出人/日時を付与**＋アラート後 `clearTry_` で毎回再アラートを停止（既読は保持=再取込しない）。clasp push済(scriptId 1SCKZ5…)＝次の15分実行で誤報停止（NJH37402等は既存→__SKIP__→clearTryで自動カウンタ解除）。
- **教訓**：GAS取込の「成功/失敗」判定関数が、**"意図的スキップ(対象外/既取込)"と"真の失敗(DBエラー/パース不能)"を同じ空値で返す**と、正しいスキップが失敗扱い→8回リトライ→誤アラートになる。skip系は必ずsentinel等で区別し呼び出し側でclearTryする。手動投入した予約はリレーが「既存」で拾うので、この区別が無いと必ず誤報る。「◯回失敗」アラートはまず実害(取込漏れ)の有無をDB(bt_reservations)で確認し、漏れが無ければ誤報＝判定ロジックを疑う。

## 📥 2026-09-25 高松じゃらんが受信BOX(?cb=bud1)に出ない根治＝取込EFが生メール台帳に保存していなかった（Gmail追加読取ゼロで対応）
オーナー「直近高松じゃらんから予約が増えてるがメールキャッチしてない」。**切り分け＝予約は取込済み(実害ゼロ)・生メール台帳だけ欠落**：`bt_reservations`のじゃらんはR0OZ2JKB/R0TLG70O/R0CMD8L3と増えているが、`bt_reservation_emails`(受信BOX ?cb=bud1 の元)のじゃらんは**9/9で止まっていた**(たびらい/エアトリ/RDCは継続保存)。
- **真因＝高松じゃらんの取込経路が変わっていた**：高松じゃらんは今 `reserve@rent-handyman.com`→(お名前転送)→noritaka.oshita Gmail→**札幌GAS(gas/Code.js・30分)→bt-import EF**で取込む(2026-09-23 楽天と同経路)。だが**bt-import EFは`bt_reservation_emails`に保存せず、札幌GASも生body(rawBody)を送っていなかった**→予約は入るが受信BOXに出ない。relay(buddicatourism読取)/torikomi のsaveRawEmailBt_ はローカルにあるが、この経路は通らない(高松じゃらんはbuddicatourismでなくrent-handyman.com宛)。
- **恒久対応(Gmail追加読取ゼロ＝クォータ問題を再発させない)**：①札幌GAS bt-import POSTに**既に読み込み済みの`body`と`subject`を`rawBody`/`subject`として付与**(Gmail再読取なし)②bt-import EF(index.ts)が予約insert成功後に`if(body.rawBody) bpost("bt_reservation_emails",{reservation_id,ota,subject,raw_body,received_at})`で台帳へ保存(BT_SR・try/catchで本処理を止めない)。→ **今後の高松じゃらん/楽天は自動で受信BOXに出る**(同経路の楽天も同時にカバー)。deno check/clasp push/EF deploy済。
- **欠落3件はGmail実メールでバックフィル済み**：高松じゃらんは`reserve@rent-handyman.com`宛でnoritaka Gmailに届く(=Gmail MCPで読める)→`from:jalan-rentacar.jalan.net`で本文取得→`bt_reservation_emails`にupsert(R0CMD8L3/R0TLG70O/R0OZ2JKB)。
- **教訓**：①「メールキャッチしてない」系はまず予約が`bt_reservations`に入っているか確認(入っていれば取込は正常＝生メール台帳の可視化gap)。②取込経路が変わった時(relay/torikomi→札幌GAS+bt-import)、**生メール台帳の保存hookも新経路に移す**(旧経路のsaveRawEmailBt_は通らない)。③生メール保存は「取込GASが既に読んだbodyを転送」で行う＝**可視化目的で新たにGmail検索を足さない**(2026-09-24のクォータ枯渇の教訓)。④高松じゃらんの実メールは`reserve@rent-handyman.com`宛=noritaka Gmailで読める(バックフィル可)／高松たびらい/エアトリ/RDC/BUDDICA自社HPは buddicatourism宛=私からは読めない(relay/torikomiのbt_reservation_emailsに依存)。

## 🚐 2026-09-25 高松じゃらんの送迎区分が「デリバリー(DEL)」になる根治＝高松は空港送迎モデル（便あり→お迎えPU/便なし→来店）
高松じゃらん3件(R0TLG70O/R0CMD8L3/R0OZ2JKB)の予約通知で**送迎=DEL(デリバリー)**になっていた。**高松空港店は無料空港送迎モデルでデリバリーは無い**(CLAUDE.md 2026-09-06)。楽天HDM高松は全件visit=PU(空港お迎え)で正しいのに、じゃらんHDMだけDEL＝誤り。
- **原因**：じゃらん高松は今 `reserve@rent-handyman.com`→札幌GAS→**bt-import EF**経由で取込む(2026-09-25)。この経路は**札幌のparseJalan_(札幌=デリバリー専門モデル)を通す**ため`visit_type=DEL`が付く。楽天は元からPUだったので露見しなかった。DELだと到着便GK449のお客様が「空港お迎え手配なし/デリバリー扱い」になり、空港で送迎を待つ客に車が届かないservice risk。
- **根治**：bt-import EF(index.ts)で**高松は便の有無からvisit_type/return_typeを導出**(`_hasFl=/[A-Za-z]{1,3}\s*\d{2,}/.test(flight)&&!/利用なし|未定|来店|利用せず/`→便あり=PU/BD、便なし=来店/返却)＝札幌パーサーのDELを上書き。楽天HDM(便あり)も同ロジックでPU維持。既存3件はDB直PATCHで是正(便あり2→PU/BD、来店1→来店/返却)。
- **教訓**：①ある店(高松)の予約を別店(札幌)のGAS/パーサー経由で取り込むと、そのパーサーの店モデル(札幌=デリバリー)が混入する(visit_type=DEL)→**取込先(bt-import EF)で取込先店のモデルに正規化**する。②送迎区分は店モデルで決まる：高松=空港送迎(便あり→PU/便なし→来店・DELにしない)/札幌=デリバリー(DEL)/那覇=送迎+デリバリー。③「予約が来た」通知の送迎/車両/ブランドは実データ(bt_reservations visit_type/brand・配車車のbt_vehicles.brand)で必ず突合(今回ブランドはHDM車でHDM予約=正・visit_typeだけ誤り)。

## 📥 2026-09-25(続) 予約受信BOX .com高松HDMタブに高松じゃらんが出ない→ミラーをゼロGmailで復活＋タブ源の整理
オーナー「.com(高松HDM)タブにR0TLG70O/R0OZ2JKBが入ってない」。前項でbt_reservation_emails(=BUDDICA高松タブ)にバックフィルしたが、**.com高松HDMタブは`reserve_inbox_mirror`(to_addrに.com含む行)を読む別ソース**だった(inquiry_deploy/index.html ResvInboxView L2519-2533：.com=reserve_inbox_mirror/.jp=同(to_addr .com以外)/BUDDICA=bt_reservation_emails)。
- **根因＝2026-09-24に私がクォータ枯渇で`mirrorReserveInbox_()`を撤去→reserve_inbox_mirrorが9/24午後で止まり.com/.jpタブが更新されなくなった**（撤去の副作用）。R0CMD8L3(9/23)は撤去前で.comに在り、R0TLG70O/R0OZ2JKB(9/24午後)は撤去後で欠落。
- **恒久対応＝ミラーを「ゼロGmail」で復活(クォータ問題を再発させない設計)**：旧ミラーの元凶は**別`GmailApp.search(150)`を毎回実行**したこと。→ その別検索を廃し、**取込ループ(processNewEmails)が既に読み込んだ`messages[j]`から`mirrorRowFor_(m)`で1行生成→ループ後に1回だけ`reserve_inbox_mirror`へ一括upsert**（新Gmail検索ゼロ＝取込本体のクォータを1も食わない・REST POSTのみ）。→ .com(高松HDM)/.jp(札幌那覇)が自動更新に復帰。
- **タブ源の整理(二重表示回避)**：高松HDM(楽天/じゃらん)は`.com`(reserve_inbox_mirror)が正→bt-import EFのbt_reservation_emails保存は撤回、バックフィルした3件もbt_reservation_emailsから削除しreserve_inbox_mirrorへ移送。**BUDDICA高松タブ=bt_reservation_emails(たびらい/エアトリ/RDC/自社HP・relay/torikomi保存)／.com高松HDMタブ=reserve_inbox_mirror(楽天/じゃらん・札幌GAS mirror-in-loop)** で1チャネル1ソースに。
- **教訓**：①可視化テーブルの「feeder(供給源)」を止めると、そのテーブルを読むUIタブが静かに更新停止する→撤去時は「何がこのテーブルを更新していたか/止めると何が止まるか」を確認。②可視化ミラーが要る時は**取込ループが既に読んだデータを再利用**(新Gmail検索を足さない)＝クォータ非消費。別`GmailApp.search`を足すのが枯渇の原因(2026-09-24)。③同じ生メールを複数タブ源(reserve_inbox_mirror/bt_reservation_emails)に入れると二重表示→1チャネル1ソースを守る。④「BOXに出ない」は"どのタブがどのテーブルを読むか"(ResvInboxViewのbox分岐)を先に確認してから正しいテーブルにバックフィルする(bt_reservation_emailsに入れても.comタブには出ない)。

## 🔒🚗 2026-09-25【全店統一の絶対ルール】顧客サイトの在庫表示＝配車表と完全連動（稼働/未稼働＋メンテ売り止め＋予約）＝オーナー確定・最上位・全店恒久
オーナー確定：「**あるべき形は配車表。メンテなどは配車表で調整するから＝メンテ＝売り止め。全店カスタマー側の在庫表示は配車表の稼働/未稼働と連動**」。公式サイト(rent-handyman.com official-flow.html)の顧客検索の空車在庫は、**配車表(FleetTimeline)が示す通り**でなければならない＝スタッフが配車表で調整した内容（稼働トグル・メンテ・予約）が顧客在庫に必ず反映される。
- **在庫の判定式（全店統一）＝ある車がその日程で顧客に売れる ⇔ 次の全てを満たす**：①**稼働中**＝配車表の稼働トグル(月別`{store}_vehicle_monthly_kpi`)が優先。kpi行があればその`active`、無ければ`vehicles.active`(全体フラグ)＝`effective_active = coalesce(kpi[貸出月].active, vehicles.active)=true` ②**予約なし**＝`{store}_fleet`×`{store}_reservations`で期間重複なし(cancel除外) ③**メンテなし＝売り止め**＝`{store}_maintenance`で期間重複なし(札幌は`block_type<>'partner_reserved'`除外＝協力会社枠は売り止めにしない)。
- **各店の実装（在庫ソース）**：
  - **高松HDM**＝RPC`public_bt_avail(p_lend,p_ret)`(BT DB)。**2026-09-25修正：従来`v.active`(全体)のみ→`coalesce(kpi[月].active, v.active)`に変更**＝配車表の月別稼働トグルを反映(仮登録でglobal active=OFFだが月別ONの車が売れない不整合を根治。例:Gクラス アクア5台が11月稼働ONなのに顧客検索に出なかった→G=5に復活)。brand=HDM絞込＋予約`not exists`＋bt_maintenance`not exists`。
  - **札幌**＝official-flow `fetchAvail`が `public_vehicles_v`(active車)−`public_busy_v`(予約+メンテUNION・売り止め済)−`public_inactive_v`(月別kpi active=false＝除外)。
  - **那覇**＝同型で`public_vehicles_nha_v`/`public_busy_nha_v`(nha_maintenance UNION済)/`public_inactive_nha_v`。
- **メンテ＝売り止めは3店とも実装済**（public_busy_v/public_busy_nha_v がメンテをUNION・public_bt_avail がbt_maintenanceをnot exists）＝配車表でメンテを入れた瞬間に顧客サイトで売れなくなる。
- **⚠️那覇/札幌の残る論点（現状該当0台・将来の番人）**：札幌/那覇の`public_vehicles_v/nha_v`は`active=true`(全体)で母集団を作る→**「全体active=OFFだが配車表で月別ON」の車は顧客在庫に出ない**（高松と同型のON方向の穴）。現状 那覇/札幌とも全体active=OFF車は**0台**＝実害なし。運用規約＝**那覇/札幌は`vehicles.active=true`(全体稼働)のまま、月ごとの除外は配車表の月別トグル(kpi)で行う**（OFF方向は`public_inactive_v`で正しく連動）。もし将来global-off車を月別ONで売りたくなったら、高松と同じく在庫ソースを`coalesce(kpi[月].active,active)`優先に直す（テンプレ＝public_bt_avail 2026-09-25）。
- **判定の鉄則**：顧客サイトで「在庫が出ない/出る」の照会は、必ず①配車表の稼働トグル(kpi)②予約③メンテ の3つを実データで突合してから答える。「配車表に在庫があるのに顧客検索に出ない」は、その車が(a)全体active=OFFで月別ONを在庫ソースが見ていない(高松型) か (b)実は予約/メンテで埋まっている、のどちらか。「配車表＝正」を絶対基準にする。
