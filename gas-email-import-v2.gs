...
/**
 * 予約番号を指定して、その予約メールを強制的に再取込する。
 * PROCESSED_MSG_IDS にあっても除外して、必ず processMessage_() を呼ぶ。
 *
 * 使い方（GASエディタ手動実行）:
 *   1. 関数を選択: forceReprocessByResvNo
 *   2. 実行前にコード内で RESV_NO を書き換えるか、
 *      `PropertiesService.getScriptProperties().setProperty('FORCE_REPROCESS_RESV', 'DY00000000946')` を先に実行
 *
 * 対象範囲: 過去 7日以内のメール全 OTA（skyticket/楽天/じゃらん/エアトリ/HP）
 */
function forceReprocessByResvNo() {
  var resvNo = PropertiesService.getScriptProperties().getProperty('FORCE_REPROCESS_RESV');
  if (!resvNo) {
    Logger.log('❌ FORCE_REPROCESS_RESV プロパティ未設定。先に以下を実行:');
    Logger.log('    PropertiesService.getScriptProperties().setProperty("FORCE_REPROCESS_RESV", "DY00000000946")');
    return;
  }
  Logger.log('=== forceReprocessByResvNo: ' + resvNo + ' ===');

  var fromClause = Object.values(OTA_SENDERS).map(function(s) { return 'from:' + s; }).join(' OR ');
  var query = '(' + fromClause + ') "' + resvNo + '" newer_than:7d';
  var threads = GmailApp.search(query, 0, 20);
  Logger.log('Gmail hit: ' + threads.length + 'スレッド');

  if (!threads.length) {
    Logger.log('❌ 予約番号 ' + resvNo + ' を含むOTAメールが過去7日以内に見つかりません。');
    Logger.log('   → Gmail で "' + resvNo + '" を手動検索して実在確認してください。');
    return;
  }

  // PROCESSED_MSG_IDS から該当メッセージIDを除去
  var processedIds = getProcessedMsgIds_();
  var removedCount = 0;
  var targetMsgs = [];
  for (var i = 0; i < threads.length; i++) {
    ...
    for (var j = 0; j < msgs.length; j++) {
      ...
      if (body.indexOf(resvNo) === -1) continue;
      targetMsgs.push(msg);
      if (processedIds[msg.getId()]) {
        delete processedIds[msg.getId()];
        removedCount++;
      }
    }
  }
  ...
  if (removedCount > 0) {
    var allIds = Object.keys(processedIds);
    PropertiesService.getScriptProperties().setProperty(PROCESSED_IDS_KEY, allIds.join(','));
    Logger.log('PROCESSED_MSG_IDS から ' + removedCount + ' 件を除去');
  }

  // 処理実行
  var successes = [], failures = [], cancellations = [], skipped = [];
  for (var k = 0; k < targetMsgs.length; k++) {
    try {
      var result = processMessage_(targetMsgs[k], false);
      if (result) {
        if (result.type === 'success')      successes.push(result);
        else if (result.type === 'failure') failures.push(result);
        else if (result.type === 'cancel')  cancellations.push(result);
      ...
      failures.push({id: resvNo, ota: '?', name: '', reason: 'エラー: ' + e.message});
    }
  }
  ...
  Logger.log('=== 結果: success=' + successes.length +
             ' / failure=' + failures.length +
             ' / cancel=' + cancellations.length +
             ' / skip=' + skipped.length + ' ===');

  // 完了後は自動でプロパティをクリア（誤実行防止）
  PropertiesService.getScriptProperties().deleteProperty('FORCE_REPROCESS_RESV');
  Logger.log('FORCE_REPROCESS_RESV プロパティをクリアしました');
}

/**
 * GAS が取りこぼしているOTA予約を自動検知して再取込する
 * （watchdog: 毎時 or 毎朝 トリガー用途）
 *
 * ロジック:
 *   1. 過去 2日以内の全OTAメールを Gmail から収集
 *   2. 各メールの body から予約番号を抽出
 *   3. Supabase reservations にその ID が存在しなければ「取りこぼし」と判定
 *   4. PROCESSED_MSG_IDS から該当を除去して processMessage_() を呼ぶ
 *   5. 結果を Slack #sapporo_reservation に通知
 *
 * 手動実行: watchdogMissedReservations()
 * トリガー設定: setupWatchdogMissedTrigger() を 1 回だけ実行
 */
function watchdogMissedReservations() {
  var fromClause = Object.values(OTA_SENDERS).map(function(s) { return 'from:' + s; }).join(' OR ');
  var query = '(' + fromClause + ') newer_than:2d';
  var threads = GmailApp.search(query, 0, 100);

  Logger.log('[watchdog] Gmail スレッド数: ' + threads.length);
  if (!threads.length) return;

  // Supabase から最近2日以内に created_at の予約を一括取得
  var since = new Date(Date.now() - 2 * 24 * 3600 * 1000).toISOString();
  var existing = supabaseGet_('reservations', 'created_at=gte.' + since + '&select=id');
  var existIds = {};
  existing.forEach(function(r) { existIds[r.id] = true; });

  var missed = [];
  var idPattern = /(DY\d{11}|R[0-9A-Z]{7}|RC\d{17}|C\d{9}|NFJ\d{5}|QAA\d{5}|[A-Z]{3}\d{5})/;
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    for (var j = 0; j < msgs.length; j++) {
      ...
      // キャンセルメールはスキップ
      var isCancel = CANCEL_KEYWORDS.some(function(kw) { return subject.indexOf(kw) !== -1; });
      if (isCancel) continue;

      // 予約番号を抽出
      var m = body.match(idPattern);
      if (!m) continue;
      var resvNo = m[1];
      ...
      missed.push({ resvNo: resvNo, msg: msg });
    }
  }

  Logger.log('[watchdog] 取りこぼし候補: ' + missed.length + '件');
  ...
  for (var k = 0; k < missed.length; k++) {
    ...
    try {
      ...
      reprocessedFail.push({id: item.resvNo, reason: e.message});
    }
  }
...
function setupWatchdogMissedTrigger() {
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'watchdogMissedReservations') {
      ScriptApp.deleteTrigger(t);
    }
  });
  ScriptApp.newTrigger('watchdogMissedReservations')
    .timeBased()
    .everyHours(1)
    .create();
  Logger.log('✅ watchdogMissedReservations 毎時トリガー設定完了');
}
