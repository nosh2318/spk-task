...
- PC版の「左ネイビー面＋黄色の斜め境界＋右写真」は変更なし
- スマホ版で写真全面＋暗いグラデーションへ別デザイン化していたCSSを廃止
- スマホでは縦長写真を上段、ネイビーのコピー面を下段へ積み、黄色の斜め境界・HDM透かし・コピーを維持
- `official-assets/hv1.jpg`〜`hv5.jpg` は縦長画像を下揃えで表示し、人物・車両・道路など主要被写体を優先
- 人物スライド `hs1` / `hs3` はスマホ時のみ顔位置へ焦点を上げ、人物の顔がネイビー面や画面外へ隠れないよう個別調整
- ヒーローのビジュアル高さと検索フォームを分離し、固定高＋`overflow:hidden`で検索フォームが欠ける問題を回避
- 390px以下は高さ610px・写真部418pxへ固定し、iPhone幅でもコピー領域を確保

## 2026-06-28 那覇専用 価格最適化ページ 空き台数修正
...
- 翌日に貸出開始する予約（`reservation.start_date = 当日+1日`）へ配車済みの車両も、洗車・準備対象として当日の空きから除外。貸出中・メンテ中との重複は二重控除しない
- カレンダーと日別モーダルの分母・クラス別「稼働対象／貸出中／メンテ中／翌日予約／空き」台数を月・日ごとに可変表示
- 本体管理APPのSupabaseログインセッションを再利用し、直接URLを開いて未ログインなら既存の共通メンバー認証を実行。取得失敗時は0台表示にせず、埋込固定データへ安全にフォールバック
- 現価格は予約実績の有無から推測せず、同一オリジンの那覇価格マスター `localStorage.hdm_nha_seasonal_v1` を正本として全4か月（2026年7〜10月）・全日付へ反映。優先順位は日別上書き→月別tier×クラス→共通tier×クラス→基本価格×tier係数
- 市場位置は日付ごとのtierに対応する5OTA市場値（最安・5位・10位）と価格マスターの現価格を比較して再計算し、先手は稼働率50%×市場位置で全日付連動
- 那覇価格マスターが端末にない場合は、空欄日のみ `DATA[クラス|tier].levels` の自社実績最多価格で補完する。予約実績ゼロ日の空欄を放置せず、市場位置・先手まで全日付で算出する
