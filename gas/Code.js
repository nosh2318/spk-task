...
function processMessage_(message, dryRun) {
  ...
  if (isCancellation) {
    if (/_TAK/i.test(body) || body.indexOf('高松空港店') !== -1) {
      var tkmId = (ota === 'rakuten')
        ? (extractField_(body, '・予約番号') || extractField_(body, '予約番号'))
        : (extractField_(body, '予約番号') || extractField_(body, '予約ID'));
      if (!dryRun && tkmId) {
        try {
          UrlFetchApp.fetch('https://ckrxttbnawkclshczsia.supabase.co/functions/v1/bt-import', {
            method:'post', contentType:'application/json', muteHttpExceptions:true,
            payload: JSON.stringify({ secret:'ecc9095ee0900b7e27fb72007205bef67b2a735bd0149d2b', isCancel:true, otaCode:otaCode, reservation:{ id: tkmId } })
          });
        } catch(e) { Logger.log('BT cancel err: '+e); }
      }
      return {type:'takamatsu_cancel', id:tkmId};
    }
  ...
  // ★2026-09-23 高松HDM(_TAK/高松空港店)→ BT直接取込(bt-import EF)。reserve.com転送先をnoritaka.oshitaに変更しこのGASが読む。
  if (/_TAK/i.test(body) || body.indexOf('高松空港店') !== -1) {
    if (!dryRun) {
      try {
        UrlFetchApp.fetch('https://ckrxttbnawkclshczsia.supabase.co/functions/v1/bt-import', {
          method:'post', contentType:'application/json', muteHttpExceptions:true,
          payload: JSON.stringify({ secret:'ecc9095ee0900b7e27fb72007205bef67b2a735bd0149d2b', otaCode: otaCode, reservation: reservation })
        });
      } catch(e) { Logger.log('BT import err: '+e); }
    }
    Logger.log('Takamatsu -> BT: ' + reservation.id);
    return {type:'takamatsu', id:reservation.id, ota:otaCode};
  }
...
// 2026-09-16 直近に発行したSquareリンクの order_id（webhook消し込みの突合キー）。
// createSquarePaymentLink_ が返り値URLを返した直後に呼び出し元が読む（GASは同期・単一スレッドなので安全）。
var _lastSquareOrderId = '';
function createSquarePaymentLink_(itemName, amount) {
  ...
  try {
    ...
    if (data.payment_link && data.payment_link.url) {
      _lastSquareOrderId = data.payment_link.order_id || '';  // ★webhook消し込み用にorder_id保持
      Logger.log('[Square] Link created: ' + data.payment_link.url + ' order=' + _lastSquareOrderId);
...
function handleJalanPayment_(reservation) {
  ...
  var payData = {reservation_id:resId, customer_name:reservation.name, customer_email:reservation.mail||'', amount:reservation.price||0, status:'link_created', square_payment_url:payUrl, square_order_id:_lastSquareOrderId, link_created_at:now, lend_date:reservation.lend_date, return_date:reservation.return_date, vehicle_class:reservation.vehicle||''};
...
function checkSquareLinks() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (pay.status === 'new') {
      ...
      var now = new Date().toISOString();
      supabaseUpdate_('jalan_payments', 'reservation_id=eq.' + encodeURIComponent(pay.reservation_id), {square_payment_url:payUrl, square_order_id:_lastSquareOrderId, status:'link_created', link_created_at:now});
    ...
    if (pay.status === 'link_created' && pay.square_payment_url && pay.customer_email) {
      var sent = true; // 2026-09-16 じゃらん初動は.com welcome(決済リンク内包)へ移行=個別送信廃止
      if (sent) {
...
function sendReservationWelcomeEmail_(reservation) {
  return false; // 2026-09-16 skyticket/エアトリ初動は.com welcomeへ移行=停止
  try {
...
function sendFormReminderEmails() {
  return; // 2026-09-16 お届け情報リマインドはofficial-notify(.com)場所リマインドへ移行=停止
  ...
  try {
    ...
    (rows || []).forEach(function(r){
      ...
      if (/cancel|キャンセル/i.test(String(r.status||''))) return;                        // キャンセル(全表記)→対象外
...
function previewFormReminders() {
  ...
  (rows||[]).forEach(function(r){ var mail=(r.mail||'').trim();
    ...
    if(((r.del_place||'').trim())||((r.col_place||'').trim()))return; if(answered[r.id])return; if(String(r.ota||'').toUpperCase()==='KEYDROP')return; if(/cancel|キャンセル/i.test(String(r.status||'')))return; if((r.lend_date||'')&&r.lend_date<todayStr)return;
...
function checkUnpaidAlert() {
  return; // 2026-09-16 スプシ廃止＝旧スプシ駆動の未入金アラートを停止。DB駆動のEF payment-unpaid-alert(毎朝9時cron)に置換(誤アラート/二重防止)。
...
function appendToPaymentSheet_(pay, payUrl) {
  return; // 2026-09-16 スプシ廃止＝じゃらんの支払い管理シート書込を停止(正本=DB jalan_payments・消し込みはSquareウェブフック)。完全スプシゼロ。
  try {
    var sheetId = '1-QU8JwrGgwp9CcZT6QieYQH0y112Hb4I5GoobrrM6tc';
    var ss = SpreadsheetApp.openById(sheetId);
    var sheet = ss.getSheetByName('支払い管理');
...
function sendSpkJalanReminderEmail_(pay) {
  return false; // 2026-09-16 じゃらん未払い催促はofficial-notify(.com)へ移行=停止
